...
float srdnoise(in vec2 P, in float rot, out vec2 grad) {
	...
	return 20.0 * (n.x + n.y + n.z);
}
void main() {

	//distortion calculation
	vec2 distortedTexCoords = texture(dudvMap, vec2(textureCoords.x, textureCoords.y)).rg*0.1;
	distortedTexCoords = textureCoords + vec2(distortedTexCoords.x + moveFactor, distortedTexCoords.y);
	vec2 totalDistortion = (texture(dudvMap, distortedTexCoords).rg * 2.0 - 1.0) * waveStrength;


	vec2 distTextureCoords = textureCoords;
	distTextureCoords += totalDistortion;

	vec4 reflectColour = texture(reflectionTexture, textureCoords);
	//vec4 refractColour = texture(refractionTexture, distTextureCoords);
	...
	//refract factor calcuation
	vec3 viewVector = normalize(toCameraVector);
	float refractiveFactor = dot(viewVector, normal);
	refractiveFactor = pow(refractiveFactor, 0.5);

	reflectColour.a = 1.0;
	finalcolor = reflectColour*0.5;// mix(refractColour, reflectColour, refractiveFactor);



	float time = moveFactor*0.8;
	vec2 g1, g2;
	vec2 p = vTexCoord2D;
	...
	vec3 seaProcedurText = vec3(n1 + 0.75*n2 + 0.5*n3);

	//finalcolor = vec4(vec3(0.0, 0.3, 0.6) + seaProcedurText, 1.0);// reflectColour;// +vec4(specularHighlights, 0.0);
